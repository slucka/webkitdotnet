...
/*
 * FormContentLengthWithArray
 *
 *** REMINDSMA: Why is it in SPI header?
 * Discussion:
 *   Estimates the content-length of a transmission consisting of the given bodyparts array. Shouldn't be exported.
 *
 * formArray:
 *   Same as formArray in CFReadStreamCreateWithFormArray
 * 
 * Result: 
 *  The expected transmission length for a request created with the given form array as payload.
 *
 */
SInt64 
FormContentLengthWithArray(
  CFArrayRef formArray);
